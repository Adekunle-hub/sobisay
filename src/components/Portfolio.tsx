...
const Portfolio = () => {
  ...
  return (
    ...
    <>
      <section className=' md:pt-[10rem]  pt-[2rem] pb-[3rem] flex items-center flex-col justify-center mx-auto w-[90%]'>
      ...
      {toggleDisplay ==='option1' && webDevelopment?.map((data,index)=>(
           <div key={index} className='border-1 border-[#ECECEC] p-5 py-[1rem] rounded-2xl '>
            <div className='relative'>
              <img src={data.image} className='w-full rounded-xl h-[18rem] object-cover  ' />
              <img src={data.arrow} className='absolute cursor-pointer -bottom-3 -right-4 bg-blue-700 rounded-[50%] p-2'/>
            </div>
        ...
        {toggleDisplay === 'option2' && videoCreatives.map((videoData, index)=>(
          <div key={index} className='border-1 border-[#ECECEC] p-5 py-[1rem] rounded-2xl '>
          <div className='relative'>
            <img src={videoData.video} className='w-full rounded-xl h-[18rem] object-cover  ' />
